  ...
  <main class="w-screen text-slate-300 z-50 absolute mt-3 pt-16 px-0 font-bold">
    <div class="flex items-center my-48 mt-80 rounded-md justify-center lg:mr-5 lg:mb-96 lg:mt-48">
      <header
        ...
        <p>{{$local->landing_body ?? 'My Portfolio' }}</p>
      </header>
    </div>
    ...
    <div class="flex items-center mx-5 my-48 sky-tr flex-col rounded-md md:w-1/2 md:ml-4 md:my-96 backdrop-blur-sm">
      <blockquote class="pt-6 px-6 text-base font-bold md:text-3xl">
        <p>{{$local->intro_text ?? "I was born in 2003, now i'm student of University Multi Data Palembang" }}</p>
        <br />
      </blockquote>
      <a class="bg-white text-black p-3 rounded-xl mb-5 flex justify-center items-center w-2/5 mr-auto ml-4 cursor-pointer hover:bg-slate-600 hover:text-white md:text-2xl"
        href={{$local->contact_me_link ?? '#socmed' }}}">{{$local->contact_me_text ?? 'Contact Me' }}</a>
    </div>
    ...
    <section
      ...
      <h2 class="text-2xl mb-3 md:text-3xl">{{$local->bio_title ?? '📜 Biography 📜' }}</h2>
      <p class="leading-7 my-3 text-base md:text-xl">
        {{$local->bio_body ?? "Hello, I'm Jelvin Krisna Putra i was born at Palembang city in 2003, my interest with
        coding and technolog
        started at young age, and has become my life-long love. On this website you will discover some of my simple
        projects that created with my main languange Javascript, after year i was learn about basic of html, css, and
    ...
    <section class="sky-tr backdrop-blur-sm flex flex-col m-5 p-5 rounded-md shadow-md md:my-96 container mx-auto">
      <h2 class="text-2xl text-center">{{$local->project_title ?? '🚀 My Projects 🚀' }}</h2>
      <div
        ...
        <div class="flex justify-center items-center gap-4 w-full">
          <div class="flex flex-col justify-center items-center gap-4">
            <p class="text-center">{{$project->title ?? 'Simple Notes Management (CRUD App, React, Typescript, Tailwind,
              Firebase)' }}
            </p>
            <a href="{{$project->link_github ?? " https://github.com/krisna31/sinodes-tailwind" }}"
              title="Link Project {{$project->title ?? ' Sinodes Tailwind' }}" rel="noopener noreferrer" target="_blank"
              class="transition-all text-center hover:text-black"><svg xmlns="http://www.w3.org/2000/svg" width="24"
                height="24" viewBox="0 0 24 24" class="text-center">
                <path
          ...
          <img src="{{ asset('storage/project/'.$project->image) }}" alt="First Project" class="w-2/3 h-1/2" />
        </div>
        @endforeach
      </div>
    </section>

    <div class="flex items-center mx-5 my-48 sky-tr flex-col md:mx-52 lg:my-96 lg:mx-80 rounded-md backdrop-blur-sm">
      <blockquote class="p-6 text-base font-bold lg:text-xl">
        <p>
          "{{$local->quote_text ?? "Programming isn't about what you know; it's about what you can figure out." }}”
          <br />
          <br />
          -{{$local->quote_name ?? 'Chris Pine' }}
        </p>
      </blockquote>
    </div>
    ...
    <div id="socmed"
      ...
      <blockquote class="p-6 text-base font-bold">
        <h2 class="text-2xl mb-3 lg:text-3xl">{{$local->social_text ?? 'Social Media' }}</h2>
        <div class="flex flex-row justify-between">
          <a href="https://www.facebook.com/profile.php?id=100006248929202" title="Link Facebook Krisna"
            class="text-gray-500 hover:text-gray-900 dark:hover:text-white" target="_blank" rel="noopener noreferrer">
            <svg class="w-5 h-4 md:w-7 md:h-7 lg:w-9 lg:h-9 text-blue-600" fill="currentColor" viewBox="0 0 24 24"
              aria-hidden="true">
          ...
          <a href="https://github.com/krisna31" title="Link Github Krisna"
            class="text-gray-500 hover:text-gray-900 dark:hover:text-white" target="_blank" rel="noopener noreferrer">
            <svg class="w-5 h-4 md:w-7 md:h-7 lg:w-9 lg:h-9 text-black" fill="currentColor" viewBox="0 0 24 24"
              aria-hidden="true">
              <path fillRule="evenodd"
          ...
          <a href="https://www.instagram.com/krisna_putra31/" title="Link Instagram Krisna"
            class="text-gray-500 hover:text-gray-900 dark:hover:text-white" target="_blank" rel="noopener noreferrer">
            <svg class="w-5 h-4 md:w-7 md:h-7 lg:w-9 lg:h-9 text-pink-600" fill="currentColor" viewBox="0 0 24 24"
              aria-hidden="true">
              <path fillRule="evenodd"
    ...
    <footer class="w-screen py-3 px-0 font-bold bg-slate-500 bg-opacity-40 backdrop-blur-sm">
      <h1
        ...
        {{$local->footer ?? 'Krisna31 | Created With Three.Js And Tailwind' }}</h1>
    </footer>
  </main>
</body>
